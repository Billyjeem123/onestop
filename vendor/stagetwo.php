...
if(isset($_GET['userId']) or isset($_GET['emailAddress']))
{
    ...
    $sql = "Select * from vendor where userId = '$userId' and emailAddress = '$emailAddress'";
    $res = $conn->query($sql);
    $out = $res->fetch_assoc();
...
<head>
    <meta charset="UTF-8">
    <title>Register Business Profile | Onestop Procurements </title>
    ...
    <script type="text/javascript">
        ...
        function sendSMSCode()
        {
			...
			var url = "storeotp.php?userId="+userId+"&otp="+otp+"&country="+country;
        ...
        function OTPCode()
        {
            ...
            var otpcode = $("#otp").val();
            var request = new XMLHttpRequest();
			var url = "verifyotp.php?userId="+userId+"&otp="+otpcode;
			request.open("GET", url, false);
			request.send();
			//alert(request.status);
			if (request.status == 200) {
                ...
                if(resp == "save") 
                {
                    window.location = 'subscription?userId='+userId+'&emailAddress='+emailAddress;
                }
                else if(resp == "error") 
                {
...
<div class="login-box">
    ...
    <div class="login-box-body">
        <p class="login-box-msg" style="color:#2B3187;font-size:16px;">Continuation of Business Profile Account </p>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" enctype="multipart/form-data" name="otpcode" >
                        ...
                        <button class="btn btn-space btn-info active btn-rounded btn-outline" type="button" name="verifyOTPCode" id="verifyOTPCode"> Verify Phone </button>
                    </form>
                </div>
            </div>
